...
- Sorting updates the current presentation immediately, then reloads through the repository ordering seam. NGA maps “最新发帖” to remote `postdatedesc` for the main forum, independent `fid/stid` channels, child aggregation and continuation pages; other sources retain their adapter behavior. Child-channel and pinned visibility changes are edited as a draft in one filter sheet and committed once on Apply.
...
- NGA latest-topic acceptance evidence (2026-07-22): after the focused real-device tests and Debug build passed, the user tested the authenticated 网事杂谈 “最新发帖” feed against live data and confirmed it was normal.
