...
网事杂谈筛选曾把 NGA 全站栏目中除当前栏目外的条目都视为子版。全站分类、收藏和标签接口不表达稳定父子关系，也会把无关栏目混入筛选。2026-07-20 的登录真机脱敏 Fixture 证明，父版主题元数据 `__F/sub_forums` 提供了可验证的直接子版结构，同时 `fid`、`stid`、服务器过滤 ID 与 attributes 具有不同含义。

### Decision

网事杂谈仅以父版 `fid=-7` 的已验证 `__F/sub_forums` 作为真实直接子版目录，不以全站栏目、名称、ID 范围或本地白名单回退。NGA Parser 严格验证父版与完整容器，并在适配层区分 `fid`/`stid` 浏览目标、过滤 ID 和 attributes；共享层只接收稳定键、标题与可浏览 `ForumChannel`。其他来源通过可选 Repository capability 明确返回无能力。

最近确认目录按来源与父版持久化，只有完整成功响应才能原子替换并计算新增、改名和取消。首次同步只建立基线；新增默认不选，改名保留选择，取消会清理有效选择并产生一次提示；失败或不完整响应保留旧快照。旧数字偏好只在权威目录可唯一映射时迁移。

网事杂谈主版始终包含，筛选只提交真实子版稳定键。聚合继续复用现有 generation、取消、远端分页、排序及 `source + tid` 去重；主版失败维持整体错误，单个子版失败保留其他成功内容并提供局部重试。本能力不调用 `nuke.php` 修改服务器端屏蔽设置。

### Consequences

- 筛选内容与服务器确认的直接父子关系一致，全站无关栏目不再泄漏到子版 UI
- `fid:N` 与 `stid:N` 不再碰撞，过滤 ID 不会被误作浏览目标
- 目录暂时失败不会误判大量子版取消，但首次无快照时只能显示目录暂不可用
- 多子版顺序聚合保持服务器友好和既有分页语义，代价是所选子版较多时可能增加加载时间
- NGA 私有响应形状变化会明确失败，需要更新脱敏 Fixture 与 Parser 契约后才能接受新结构

## ADR-020 Canonical Channel Identity Unifies Discovery, Subscription, And Navigation
...
栏目管理只消费 NGA 全站收藏、分类和标签接口，无法发现父版权威目录中的 `stid` 子版；网事杂谈筛选则已经能读取 `[股市]技术分析` 等真实直接子版。普通栏目使用裸数字 `nativeKey`，权威目录使用 `fid:`/`stid:`，直接拼接会重复同一 fid，并让数值相同的不同浏览种类在 SwiftUI 列表、当前选择和订阅持久化中碰撞。

### Decision

栏目发现使用 Feature 层只读目录快照组合普通栏目与最近确认的权威子版，不修改 Repository 原始栏目语义。组合、搜索、首页导航、横向分页、当前高亮、订阅和排序统一使用来源作用域规范化身份：NGA 普通数字与显式 fid 都是 `nga:fid:<id>`，stid 是 `nga:stid:<id>`，其他来源沿用 `source:nativeKey`。远端 Int 仅保留为请求载荷和辅助展示值。

权威子版可以作为首页独立栏目订阅并只加载自己的 Feed；该订阅与网事杂谈“主版 + 多子版”聚合筛选是两套互不联动的偏好。完整权威快照新增子版时不自动订阅，改名按稳定键保留订阅，取消时分别清理两套偏好；失败或不完整响应保留最近确认目录与订阅。

订阅存储升级到新版本并迁移旧 `nga:<数字>` 键，但不覆盖旧订阅与顺序快照。这样旧二进制回滚仍能恢复原有普通栏目；旧版本忽略无法理解的新 stid 订阅，而不能把它误作 fid。

### Consequences

- `[股市]技术分析` 等 stid 子版可以在栏目页搜索、添加、排序并从首页独立打开
- 同一 fid 在全站目录和权威目录中只展示一次，同值 fid/stid 可以安全共存
- 首页独立订阅和网事杂谈聚合筛选提供两条明确入口，状态变化不会互相污染
- 权威目录缓存成为栏目发现的依赖；首次无缓存且请求失败时只能显示普通栏目和“子版暂不可更新”状态
...
NGA 标准表情的远端 smile 域名已经发生变化，历史帖子和编辑器中的图片标记还可能携带旧域名。只按完整 URL 使用内存或 URLCache 会让同一个表情因域名变化失去缓存；只依赖远端又无法保证首次离线或远端故障时的展示。

### Decision

已确认且体积较小的 NGA 标准 smile PNG 随 App 分发，标准表情加载优先使用内置资源；未随当前版本内置但曾成功下载的 smile 资源写入 `Library/Caches/ForumEmoji/<filename>`，旧域名与当前域名共享同一文件名。只有标准 smile 路径进入该本地资源接缝，普通附件、头像和用户图片继续使用通用远程图片管线。应用清理缓存时删除运行时表情缓存，但不影响 App 内置资源。

### Consequences

- 已知标准表情不再依赖远端域名才能首次展示，远端短时失败不会阻塞帖子阅读或回复编辑
- 资源清单需要随 NGA 目录维护，新增或改名的标准表情需要随 App 更新；未知资源仍可通过远端路径尝试加载
- 内置资源增加少量 App 包体，当前已校验的 PNG 总量约 1.1 MB；服务器返回 404 的历史文件不生成伪造占位图
- 已确认返回 404 的历史文件不进入回复选择器，但仍保留目录反向识别，避免读取旧内容时改变文件名语义
- 本地缓存位于可清理目录，不能把它当作唯一离线持久化来源；内置资源才是标准表情的可靠兜底
