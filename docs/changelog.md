...
## 2026-08-07 NGA 回复表情资源地址

- 修复帖子正文和回复中的 NGA 表情仍请求旧 `img4.nga.178.com`、导致当前资源无法加载的问题，新生成地址统一改用网页版当前使用的 `https://img4.nga.cn/ngabbs/post/smile/`，旧帖路径读取时也会归一化到当前域名。
- 将 `img4.nga.cn` 纳入可信图片地址和 NGA 请求 Header 边界，并兼容已有草稿中的旧 smile 地址。
- 将已确认的标准 NGA smile PNG 随 App 分发，表情加载优先使用内置资源；未内置资源写入 `Library/Caches/ForumEmoji` 时按文件名复用旧、新域名，远端失败不会阻塞表情展示。
- 不再在回复表情选择器中展示当前资源服务返回 404 的 `ng_39.png`、`ng_40.png`、`a2_06.png`、`a2_29.png`、`a2_34.png`、`a2_35.png`；历史内容的文件名反向识别仍保留。

## 2026-07-31 首页滚动导航状态修复

- 修复首页横向频道切换与帖子点击冲突：切换手势期间临时关闭帖子命中测试，并在导航动作执行前再次校验横滑状态，横滑不再误进详情。
- 该交互保护不再使用 `.disabled`，因此纵向来回滚动不会让帖子行置灰。

## 2026-07-30 固定竖屏

- ForumHub 在 iPhone 上固定竖屏；iPad 保留正向与倒置竖屏。移除两种构建配置中的横屏方向，旋转设备不会再自动切换到横向布局。

## 2026-07-30 NGA 回帖失败反馈

- 真机实测确认竖屏“发布”点击、正文校验和登录校验均正常，且服务端已经创建回复；卡住的原因是 NGA 的最终 POST 会发送响应正文却不可靠地触发连接完成回调，导致客户端 `await` 未返回，成功关闭 Sheet 的分支没有执行。
- 回复上下文、确认读取与附件上传继续使用系统 `URLSession` 的完整响应路径；仅最终回帖 POST 使用隔离 ephemeral `URLSession`、`URLSessionDataDelegate`，并在收到响应正文静默 1 秒后主动结束读取。若写连接仍不返回，App 并行只读轮询当前帖的回复总数；确认增长即关闭编辑页、刷新详情并取消悬挂请求，数秒内不能确认则在编辑页显示明确提示。2026-07-30 已在真机真实 NGA 回帖验证：约 6 秒后自动关闭编辑页并返回详情，无卡死或闪退。
- 最终表单按网页回帖合约携带 `post_subject`，不再把仅用于附件上传的 `auth` 混入最终回帖请求；服务端拒绝文案仍原样显示。
- 修复发布时的真机 watchdog 闪退：发布状态变化不再切换富文本 `UITextView.isEditable`。该 UIKit 属性会隐式触发键盘退出并与 SwiftUI Sheet 更新循环竞争，主线程超过 10 秒后会被系统终止。
- 修复登录校验或 Repository 错误由详情页背后呈现，导致当前回复 Sheet 没有稳定可见反馈的问题。
- 回复失败现在直接在当前编辑 Sheet 内提示，并保留编辑器、回复目标、正文草稿和附件；成功发布流程保持不变。
- 已有异步提交关闭编辑页的 UI 回归场景和服务端错误文案保留单元测试；本次真实 NGA 半关闭响应已完成真机自动回归。

## 2026-07-30 浏览足迹统一记录

- 浏览足迹改为在帖子详情实际出现时统一写入，不再依赖首页列表与 `NavigationLink` 竞争的点击手势。
- 从首页、热门、搜索结果、收藏或足迹自身重新打开帖子都会按 `source + threadID` 更新最近访问记录；新增真机 UI 回归覆盖“搜索 → 详情 → 足迹”完整路径。
- 2026-07-30 用户已在真机确认足迹记录与 NGA 主题标题搜索均正常。

## 2026-07-30 NGA 混合 HTML 通用降级

- 将 NGA API 与 Web 正文原先分散的 HTML 标签正则白名单收敛为同一个容错扫描器，不再按 `foldBox`、`videoSize` 等站点 class 逐项补丁。
- 块级容器保留阅读边界，行内与未知元素去掉标签外壳并保留可读子内容；链接、延迟图片和视频统一降低为安全语义节点。
- 视频封面现在整块可点击并在帖子原位启动系统播放器，不再把封面当普通图片或把“播放视频”作为 Safari 主入口；全帖只允许一个活跃视频，切换时暂停前一个，长图则以封面、播放标记和“视频”文案安全降级。
- 2026-07-30 用户已在连接真机的真实 NGA 楼层完成封面点击与 App 内原位播放验收，确认播放正常且不会跳转 Safari。
- `script/style/template`、`onclick/onplay` 等事件属性及 `javascript:` 地址不会进入正文或成为可点击链接；原始 markup 仍只保留在 representation 中用于追溯。
- 新增依据两次用户真机截图脱敏扩展的混合 HTML Fixture，同时覆盖 API/Web 正文入口、折叠容器、列表、表格、链接、图片、视频、未知元素与不可执行内容。
...
- 权威子版可以添加为首页顶部独立栏目并参与本地排序；独立打开只加载该子版，不改变网事杂谈“主版 + 多子版”聚合筛选，筛选操作也不改变首页订阅。
- 栏目、导航、分页、当前高亮与订阅统一使用规范化来源身份。NGA 普通栏目迁移为 `nga:fid:<id>`，`stid` 保持 `nga:stid:<id>`，同值不同种类不再碰撞。
- 订阅存储升级并保留旧快照供回滚；完整目录改名保留订阅，取消分别清理首页订阅与聚合选择，失败刷新保留最近确认目录和订阅。
- 栏目页重写为“已添加到首页”和“浏览全部栏目”两类任务区域，搜索使用统一结果，打开与添加是独立操作，拖动手柄仅在“编辑排序”时显示。
- 修复 `stid` 独立栏目网页列表把回复数辅助链接误当主题的问题；解析器现在从同一 `topicrow` 的真实标题、作者、回复数和发帖时间节点建模，不再整页显示“打开新窗口”“未知作者”和“来自网页兜底解析”。
- Xcode Beta 已在“哥谭之王”真机复现并验证真实 `[股市]技术分析` 前 10 条主题元数据，随后运行完整 `ForumHubTests`（15 个套件、192 项），全部通过，Debug 真机构建及覆盖安装通过；用户确认完整流程及修复后的列表界面均正常。

## 2026-07-22 NGA 网事杂谈真实子版筛选

- 网事杂谈筛选改为只展示父版元数据确认的直接子版；主版始终包含，并支持多选真实子版或一键恢复“只看主版”。全站无关 NGA 栏目不再出现或计入筛选数量。
- 子版选择改用区分 `fid` 与 `stid` 的稳定键并按来源、父版保存；旧数字偏好只在权威目录可唯一映射时迁移。完整目录同步会识别新增、改名和取消，失败或异常响应继续使用最近确认快照。
- 新子版默认不选并显示“新”，取消的已选子版会自动清理并提示；没有确认目录时显示暂不可用，真实子版较多时可按名称或稳定键本地搜索。
- 主版与所选子版继续按远端页聚合、排序并以 `source + tid` 去重；单个子版失败不再清空已成功内容，并可单独重试，主版失败仍使用整体错误状态。
- 前台权威目录刷新若取消当前已选子版，会先使旧 Feed generation 失效，再只以剩余稳定键刷新一次，旧 in-flight 聚合结果不能覆盖新内容。
- 增加脱敏真机 `__F/sub_forums` Fixture，以及 Parser、Repository、目录快照/偏好迁移和 Feed 筛选/聚合回归测试；Xcode Beta 在“哥谭之王”真机运行 4 个测试套件共 39 项，全部通过。

## 2026-07-18 NGA 回复编辑器与表情目录

- 回复编辑器改为单一 Sheet 内的键盘/表情双模式，使用现有纸张主题与玻璃工具栏；表情目录在面板内展开，支持横向切换六组分类、连续插入和点击正文返回输入。
- 修复首次打开回帖时富文本编辑器在挂入窗口前请求焦点、导致键盘不弹出的问题；焦点现在通过每代只执行一次的异步命令，在 Sheet 完成 UIKit 挂载后获取。
- 在既有 NG娘、AC娘 v1、AC娘 v2 基础上新增潘斯特 `pt00...pt64`、外域 `dt01...dt33`、企鹅 `pg01...pg15`，目录成员由本地元数据确定，单个缩略图失败只显示局部占位。
- 富文本插入、恢复和提交统一使用 NGA 来源目录，继续输出原有 `[img]URL[/img]` 标记；未知域名或未知文件名不会被误识别为 NGA 表情。
- 修复键盘可见时回复 Sheet 被动态压缩到约 280pt、导致“回复主题”和底部快捷表情被裁切的问题；面板现在固定使用大号 detent，窄宽度工具栏自动换行，六组分类与主要操作已通过真机可达性检查、点击区域和 trait 审计。
- 修复关闭回复页时持续焦点状态重新抢占 first responder、导致 Sheet 无法退出的问题；键盘、表情、返回键盘和连续三次开关场景已通过真机关闭回归。
- 修复真实 NGA 发布请求等待时同时禁用发布和关闭、导致用户被锁在回复页的问题；发布期间现在显示明确状态，关闭按钮可取消请求并保留草稿，回复上下文/最终 POST 与附件上传分别使用 30 秒和 60 秒超时。

## 2026-07-17 V2EX 栏目与下拉刷新

- 修复首页与热门信息流下拉手势未连接到实际滚动区域的问题；下拉刷新继续复用现有首屏重载及请求代次隔离。
- 修复 V2EX 左上角“最热”滚动到底部不加载更多的问题；首屏保留每日 Top 10，后续按 PC 页“更多新主题”入口续载并去重。左上角“最热”与底栏“热门”现在保留相同的真实节点标签，不再统一显示为“最热”。
- 修复 V2EX “问与答”“二手交易”等公共节点因主题链接 class 或属性顺序变化而显示空列表的问题；节点浏览保持匿名且不携带 Token 或 Web Cookie。

## 2026-07-16 首页信息层级、筛选与时间

- 首页按来源、搜索/刷新/发帖、频道、排序/筛选、条件式登录提示和帖子列表重组；移除重复频道行，并在长列表滚动时折叠低频顶部动作。
- “最后回复 / 最新发帖”保持即时互斥排序；子版与置顶进入统一筛选 Sheet，支持草稿、激活数量、重置和一次性应用。
- NGA、V2EX、LINUX DO Feed 统一使用结构化时间，帖子行按实际字段显示“回复/发布 MM-dd HH:mm”，不再泄漏原始 Unix 或来源字符串。
- 首页登录提示只在未登录或明确失效时出现；NGA 发帖接入来源 capability 和同会话网页入口，登录后仅在来源与频道仍匹配时续接。

## 2026-07-16 V2EX 楼中楼回复

- V2EX 帖子详情会从公开 API 原始回复中的 `@username` 与可选 `#楼层` 推断父子关系，并默认按移动端楼中楼结构展示；不解析主题网页。
- 明确楼层与作者一致时优先连接；只有单一用户名时回溯最近的前序同名回复；冲突和多用户歧义安全降级，不把推断当作服务端权威关系。
- 更多菜单可在“楼中楼显示”和“平铺回帖”间切换；倒序只反转根分组，只看楼主继续使用原有精确平铺过滤。
- V2EX `content_rendered` 中的图片现在映射为语义图片块，主题与回复不再把图片降级显示为 `[图片] URL` 文本。
- 树形正文最多缩进三层，已确认的开头引用只在视觉投影中省略，完整正文、分享、截图和无障碍文本保持不变。
- 新增主题 `1227563` 的脱敏真实形状 Fixture；8 项 V2EX 聚焦测试与完整 `ForumHubTests` 135 项已在 iPhone 真机通过，并完成真实主题分支、媒体与阅读模式人工验收。
