...
- [ ] R-202 在 NGA API 解析内容不完整时以网页结果补全而非整体覆盖
  ...
  - 状态：已修改；已按真实 NGA HTML 收窄网页正文选择器，只匹配 `postcontent<楼层序号>` 节点，避免将包装器或其他内容区域误作 1L；详情“更多”可在 App 内加载 NGA、V2EX、LINUX DO 原帖网页，NGA 复用登录 Cookie；Debug 构建可复制当前原始响应用于回归。真机单元测试通过，等待真实 NGA 帖子验证。
