...
- [x] **R-001 修复 NGA 回帖确认被结构化并发阻塞**
  - 优先级：高
  - 涉及文件或模块：`ThreadDetailViewModel`、`ThreadRepository`、`NGALiveThreadRepository`
  - 状态：2026-07-30 真机真实 NGA 回帖验证通过；编辑 Sheet 自动关闭并返回帖子详情。
  - 验收标准：确认到回复总数增长时，无论原始 POST 是否完成，编辑 Sheet 都在有限时间内关闭并触发详情刷新；取消悬挂写请求不阻塞该成功路径。

- [x] **R-002 为 NGA 回帖确认建立有界读取与端到端回归**
  - 优先级：高
  - 涉及文件或模块：`NGALiveThreadRepository`、`ThreadDetailViewModel`、`ForumHubTests`、`ForumHubUITests`
  - 状态：2026-07-30 真机真实 NGA 回帖验证通过；发布后约 6 秒自动关闭编辑 Sheet，未复现卡死或 watchdog 闪退。
  - 验收标准：确认读取有独立硬超时；新增测试覆盖“POST 永不结束、确认读取成功”时自动关闭编辑 Sheet、清空草稿并刷新一次详情；确认失败时保留草稿并显示明确错误。
...
- [ ] **SD-4.1 建立未支持内容格式清单**
  ...
  - 优先级：P2
  - 状态：进行中；2026-07-27 至 2026-07-30 用户真机截图已确认 NGA API 正文中的展示 `span`、`ul/li`、`div.foldBox` 与 `video` 会泄漏为可见标签。API/Web 两入口现共用通用 HTML 容错扫描器：容器/未知标签透明保留子内容，图片、HTTPS 链接和含可选封面的正式视频块降低为共享语义节点，脚本、样式、事件属性与 `javascript:` 链接丢弃；视频已支持 App 原位点击播放与全帖单视频活跃，脱敏真实形状回归覆盖直接 `src` 和嵌套 `source`，并于 2026-07-30 通过用户真实 NGA 楼层真机播放验收。表格、代码块等独立原生保真节点仍等待真实样本，不视为本项完成。
  - 验收标准：
...
- [ ] **SD-4.2 逐类扩展内容节点与降级渲染**
  ...
  - 优先级：P2
  - 状态：等待 `SD-4.1` 完整清单；截图已证明的展示容器、列表与折叠 HTML 无需让 View 解析来源 HTML，视频已按真实证据新增共享领域节点并完成原位播放器、单视频协调、长图及纯文本降级。表格、代码块等是否需要新增领域节点仍由后续真实证据决定。
  - 验收标准：
    - 每次只增加一种节点；
...
- [ ] **SD-6.4 NGA 标准表情本地资源兜底**
  - 具体问题：标准 NGA smile 依赖远端域名，域名变化或短时不可达会导致帖子回复表情不展示。
  - 涉及文件或模块：`NGAReplyEmojiCatalog`、`NGAForumEmojiLocalStore`、`NGAImageLoader`、标准 smile 资源和图片缓存。
  - 优先级：P1
  - 状态：已修改；243 个有效 PNG 已随 App 打包，旧/新域名共享文件名缓存，相关真机聚焦测试通过。6 个当前服务器返回 404 的历史文件未生成伪造资源，且已从回复选择器过滤；完整线上帖子视觉验收仍待用户确认。
  - 验收标准：
    - 标准表情优先从 App 内置资源加载，远端失败不阻塞展示；
    - 旧域名和当前域名使用同一个文件名缓存 key；
    - 普通附件和未知图片不进入表情本地资源路径；
    - 已确认返回 404 的历史文件不出现在选择器，旧文件名仍可被目录反向识别；
    - 真机 Debug/Release 构建、聚焦测试和线上帖子人工确认通过。
...
- [x] **SD-7.6 首页信息层级、类型化会话、统一筛选与结构化时间**
  - 具体问题：首页存在重复频道信息、分散筛选入口、原始时间戳泄漏、登录状态语义不足及发帖空动作。
  - 涉及文件或模块：`ContentView`、`ForumFeed`、`ForumTime`、各来源 Adapter、Session 描述、历史/收藏迁移
  - 优先级：P1
  - 状态：已完成；真机完整 `ForumHubTests`、Debug 构建、首页聚焦 UI、超大动态字体与无障碍审计均已通过，并完成 NGA、V2EX、LINUX DO 时间、NGA 发帖入口及人工 VoiceOver 验收。
  - 验收标准：
    - 三个来源的时间统一显示为带实际字段标签的 `MM-dd HH:mm`，旧快照可恢复且技术值不泄漏；
    - 首页只在未登录或明确失效时显示正确 CTA，普通网络/403/5xx 不误判失效；
    - 排序即时，筛选草稿一次应用，偏好作用域隔离且失效子版被清理；
...
- [x] **SD-7.9 重写 NGA 栏目目录并支持权威子版独立订阅**
  - 具体问题：栏目页只消费全站普通栏目接口，无法搜索或订阅网事杂谈权威目录中的 `[股市]技术分析` 等真实子版；页面排序与全部栏目形成重复长列表，裸 Int 身份也无法安全区分同值 `fid/stid`。
  - 涉及文件或模块：`ForumChannel` 规范化身份、`ForumSubscriptionStore`、`ForumChannelCatalog`、`CommunityView`、`ContentView`、Feed 顶部导航、权威目录同步、偏好测试与文档
  - 优先级：P1
  - 状态：已完成。人工验收发现的 `stid` 网页列表辅助链接误判已用脱敏真实 `topicrow` Fixture 修复；Xcode Beta 在“哥谭之王”真机验证真实 `[股市]技术分析` 主题元数据，完整 `ForumHubTests`（15 个套件、192 项）全部通过，Debug 真机构建及覆盖安装通过。用户复查搜索、订阅、重启、排序、独立浏览、筛选隔离、可访问性及修复后列表均正常。
  - 验收标准：
    - 栏目页组合普通栏目与最近确认权威子版，搜索“技术分析”或 `stid:47206901` 可定位 `[股市]技术分析`；
    - 添加后以 `nga:stid:47206901` 出现在首页顶部，重启和排序可恢复，打开时只加载该子版；
    - 首页独立订阅与网事杂谈聚合筛选互不联动，完整取消分别安全清理，失败刷新不删除缓存订阅；
    - 旧 NGA 裸数字订阅无损迁移且保留旧快照，同值 `fid/stid` 在列表、选择、分页和导航中不碰撞；
