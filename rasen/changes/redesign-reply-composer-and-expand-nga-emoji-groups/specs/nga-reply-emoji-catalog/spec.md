...
### Requirement: 标准表情具备本地资源兜底
系统 SHALL 随 App 分发已确认 NGA 标准表情的本地 PNG 资源，并在远端 smile 资源不可用时优先使用本地资源；运行时缓存 MUST 使用表情文件名作为稳定标识，不得将远端域名作为唯一缓存身份。

#### Scenario: 远端表情不可用
- **WHEN** 已知 NGA smile 文件的远端请求失败或域名暂时不可达
- **THEN** 系统仍使用 App 内置或本地缓存的同名 PNG 渲染表情，不阻塞帖子阅读或回复编辑

#### Scenario: 域名变化后复用本地表情
- **WHEN** 同一个表情分别通过旧的 `img4.nga.178.com` 和当前的 `img4.nga.cn` 地址请求
- **THEN** 系统按同一个文件名缓存和读取资源，不因域名变化丢失本地表情

#### Scenario: 非标准图片不使用表情兜底
- **WHEN** 普通附件或未知 smile 文件没有对应的本地标准资源
- **THEN** 系统继续使用原有远程图片加载与局部失败降级，不将其误识别为内置表情
